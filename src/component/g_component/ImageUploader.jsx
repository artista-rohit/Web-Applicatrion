    ...
    <div className="max-w-md h-40 rounded-lg border-2 border-dashed flex items-center justify-center">
      {!image ? (
        <>
          <label
            htmlFor="file"
            className="cursor-pointer text-center p-4 md:p-8 w-full h-full flex flex-col items-center justify-center"
          >
            <svg
        ...
        <div className="relative w-full h-full">
          <img
            src={image}
            alt="preview"
            className="h-40 w-full object-cover border rounded-lg"
          />
          <button
            type="button"
            onClick={onRemove}
          ...
          >
            <TiDeleteOutline fontSize={32} />
          </button>
          <small className="absolute bottom-1 left-2 text-xs text-gray-600 bg-white/70 px-2 rounded">
            Preview only — image saved as base64
          </small>
        </div>
      )}
    </div>
